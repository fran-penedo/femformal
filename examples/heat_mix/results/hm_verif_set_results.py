...
prefix = "examples/heat_mix/results/hm_verif_set_simple2_results_tol{}.py"
...
def load_module(name, f):
    try:
        ...
        print("Couldn't load module {}".format(f), sys.stderr)
        traceback.print_exc()
        return None
...
if __name__ == "__main__":
    ...
    ts = []
    for tol in tols:
        f = prefix.format(str(tol).replace(".", "_"))
        name = f.replace("/", ".")
        m = load_module(name, f)
        if m is not None:
            rs.append(m.robustness)
            ts.append(m.time)
    ...
    # plt.show()
    fig.savefig(
        "temp_plots/hm_verif_set_simple2.png", bbox_inches="tight", pad_inches=0
    )
