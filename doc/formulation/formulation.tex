...
\to \R$ at time $t \in \R$ is defined as $u[t] \models \lambda_i \iff 
  ...
  X_i$ otherwise. We also define quantitative semantics as usual with
...
predicates is defined as in Sec.~\ref{sub:signal_temporal_logic}.
...
Consider a rod with parameters given in Table~\ref{tab:ex_pars}. We want to
...
temperature profile is above the profile $\mu_2(x) = 9x + 15$." Note that we used
the informal syntax for predicates to improve the readability of the formula.
...
We consider first the verification of a single initial value, $u_0(x) = 20.0$
for $x \in (0, L)$, $u_0(0) = g_0, u_0(L) = g_L$, using uniform meshes of
different sizes. We computed the bound $\delta(x) = \delta$ as the maximum pointwise
...
The results are shown in Table~\ref{tab:res_meshes}. As expected, finer meshes
...
\begin{tabular}{|c|c|c|c|c|}
    \hline
    $h$ & $\delta$ & $r(\phi_{FEM}^{0, 0, 0})$ & $r(\phi_{FEM}^{\delta, 0,
0})$ & Time (s)  \\
    \hline
    1 & 5.29 & 1.66 & -3.63 & 0.5 \\
    0.5 & 1.49 & -1.57 & -3.06 & 1.5 \\
    0.33 & 0.68 & -2.29 & -2.96 & 3.6 \\
    0.25 & 0.37 & -2.68 & -3.06 & 5.9 \\
    0.2 & 0.24 & -2.74 & -2.98 & 9.9 \\
    \hline
\end{tabular}
...
of the previous form. We used for this example a uniform mesh of size $h = 0.2$. We
show in Fig~\ref{fig:res_diffs} the difference between the true robustness and
the robustness for our mesh, both with and without the perturbation. Note that
when we include the perturbation, we correctly verify as satisfying all examples
that are satisfying. Regarding the conservatism of the approach, we obtain a
difference of about 1 degree in the robustness, which would allow us to verify a
specification that is satisfied by at least 1 degree at all points in a system
that ranges through 90 degrees. It took an average of 49.8 seconds with a
standard deviation of 2.2 to verify the perturbed formula.
...
\begin{tabular}{|c|c|c|c|c|}
    ...
    \hline
    9 & [5, 15] & 0.7 & Satisfies & 4291 \\
    9 & [0, 10] & -1.0 & Undefined & 315 \\
    \hline
\end{tabular}
