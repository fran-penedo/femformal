...
One possible problem one might want to solve during the analysis of a system is
that of verifying whether it satisfies a complex property. These properties are
commonly stated in a formal language that ensures the specification has a
non-ambiguous meaning while being expressive enough to capture many different
behaviours of the system. Among these, temporal logics have enjoyed some
...
(LTL) and Computational Tree Logic (CTL) have been widely used to verify
...
using Bounded Linear Temporal Logic (BLTL), Metric Temporal Logic (MTL), 
or Time-Window Temporal Logic (TWTL). We may additionally be interested in more
...
admits quantitative semantics that corresponds to the distance of the system's
...
needed, which can present a problem when we attempt to verify the behaviour of a
system for a set of initial conditions. 
