...
\begin{example}
    ...
    Consider the following formula in STL for PDEs with a single predicate
...
\begin{example}
    ...
    Assume we obtain a bound $\delta(x) = 1$ for some system $\Sigma$ and its
...
\begin{example}
    ...
    approximation using the mesh $\{0.5 i | i \in 0,...,20\}$ and we found the
