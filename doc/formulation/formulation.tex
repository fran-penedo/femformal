...
\label{def:delta_perturbation}
    ...
    \begin{itemize}
        \item If $\lambda$ is not preceded by a negation operator, then it is
            substituted by $\lambda^{-M}$.
        \item Otherwise, it is substituted by $\lambda^{M}$
    \end{itemize}

    In both cases, 
    
    \begin{equation}
        M(x) = \delta(x) \max_{u} \frac{\partial \mu}{\partial u} (x, u)
    \end{equation}

    We will also use $\Lambda^{\delta}$ to refer to the set of all $M$ and $-M$
...
\Lambda^{\delta}, j \in J_i\}$, where $J_i= \{j |
x_j \in X_\lambda\}$ and $\alpha_{\lambda, j}$ is defined by $d[t] \models
\alpha_{\lambda, j} \iff \mu_\lambda(x_j, d_j(t)) > 0$, for a continuous-time signal $d : [0, T] \to \R^n$.
...
\begin{definition}
    The STL formula over $\Lambda^{\delta}_{FEM}$, $\psi^{\delta, L}_{FEM}$, corresponding to an STL
    formula in negation normal form, $\psi^\delta$, over $\Lambda^\delta$, with
    $L : \bar\Omega \to \R$, is a formula obtained by substituting every
    predicate $\lambda$ in $\psi^\delta$ by the formula $\bigoplus_{j=1}^{|J_i|
    - 1} \beta_j$, where $\oplus = \wedge$ if $Q_i = \forall$ and $\oplus = \vee$
    otherwise, and $\beta_j$ is defined as the following STL formula:

    \begin{itemize}
        \item If $\delta$ is not preceded by a negation operator, then $\beta_j
            = \alpha_j^{-K_j} \vee \alpha_{j+1}^{-K_j}$.
        \item Otherwise, $\beta_j = \alpha_j^{K_j} \vee \alpha_{j+1}^{K_j}$.
    \end{itemize}
    ...
    \begin{equation}
    \begin{aligned}
        K_j = (x_{j+1} - x_j) & \left (\max_{c \in [x_j, x_{j+1}], u} \left \{ 
            \frac{\partial \mu}{\partial x}(c, u) \right \} + \right.\\
            & \left. \max_{c \in [x_j, x_{j+1}], u} \left \{ 
            \frac{\partial \mu}{\partial u}(c, u) \right \} 
            \max_{c \in [x_j, x_{j+1}]} \left \{ L(c) \right \} \right )
    \end{aligned}
    \end{equation}
\end{definition}
...
\begin{theorem}
    If $d \models \psi^{\delta, L}_{FEM}$, with $L(x) \geq |\max_t \frac{\partial
...
\fran{Next: Translate into discrete time}
