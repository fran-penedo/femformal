...
class Mesh(object):
    ...
    def _reshape_int_values(self, d):
        if d.shape[-1] % self.nnodes != 0:
            raise ValueError(
                "Interpolating values do not agree with number of "
                "nodes: nvalues = {}, nnnodes = {}".format(d.shape[-1], self.nnodes)
            )
    ...
    def interpolate_strain(self, d):
        ...
        if dofs != 2:
            raise ValueError(
                "Need dofs = 2 for strain interpolation. Given {}".format(dofs)
            )
    ...
    def build_elem(self, value):
        ...
        if self._build_elem is not None:
            self.elements = [
                self.build_elem(self.elem_coords(e)) for e in range(self.nelems)
            ]
        else:
            self.elements = None
...
class GridMesh(Mesh):
    ...
    def find_nodes_between(self, coords1, coords2):
        ...
        """
        n1, n2 = [find_node(coords, self.nodes_coords) for coords in [coords1, coords2]]
        n1s, n2s = [np.array(_unflatten_coord(n, self.shape)) for n in [n1, n2]]
        nodes = [
            _flatten_coord(n1s + off, self.shape)
    ...
    def find_containing_elem(self, coords):
        for e in range(self.nelems):
            left, right = [self.nodes_coords[self.elems_nodes[e][p]] for p in [0, 2]]
            if np.all(coords >= left) and np.all(coords <= right):
                return e
        raise ValueError("Coordinates outside the domain. Given {}".format(coords))
    ...
    def find_border_full_elems(self):
        ...
        for axis in range(len(border_coords)):
            for i in [0, -1]:
                coords_list = (
                    [border_coords[j][1:-1] for j in range(axis)]
                    + [[border_coords[axis][i]]]
                    + [border_coords[j] for j in range(axis + 1, len(border_coords))]
                )
...
class GridMesh2D(GridMesh):
    ...
    def find_elems_between(self, coords1, coords2):
        ...
        if n1 == n2:
            ...
            try:
                e1, e2 = [
                    find_elem_with_vertex(n, pos, self._elems1dh_nodes)
                    for n, pos in zip([n1, n2], [0, 2])
                ]
            except ValueError as e:
                logger.error(
                    "Given coordinates not aligned with element "
                    "partition. Given {}".format([coords1, coords2])
                )
                raise e
            return ElementSet(1, {e: self.elem_coords(e, 1) for e in range(e1, e2 + 1)})
        elif self._invline(n1, n2):
            try:
                e1, e2 = [
                    self._num_elems1dh(self.shape)
            ...
            try:
                e1, e2 = [
                    find_elem_with_vertex(n, pos, self.elems_nodes)
                    for n, pos in zip([n1, n2], [0, 2])
                ]
            except ValueError as e:
                logger.error(
            ...
            return ElementSet(
                2,
                {
                    i * nelemsx + j: self.elem_coords(i * nelemsx + j, 2)
...
class GridQ4(GridMesh2D):
    ...
    def __init__(self, partitions, build_elem):
        ...
        self.elems_nodes = np.array(
            [GridQ4._elem_nodes(e, self.shape[0] - 1) for e in range(self.nelems)]
        )
...
class GridQ9(GridMesh2D):
    ...
    def _elem1d_nodes(e, shape):
        ...
        if e >= nelems1dh:
            ...
            x0 = 2 * e + e // nelems1d_x
            return np.array(
                [x0, x0 + 2, x0 + 2, x0, x0 + 1, x0 + 2, x0 + 1, x0, x0 + 1]
            )
...
class ElementSet(object):
    ...
    def intersection(self, other):
        ...
        if self.dimension != other.dimension:
            raise ValueError(
                "Intersection undefined for sets of elements of different dimension"
            )
        else:
            elem_coords_map = {
                e: self[e]
                for e in self.elem_coords_map.viewkeys()
    ...
    def __eq__(self, other):
        if isinstance(other, self.__class__):
            if self.dimension != other.dimension or len(self.elems) != len(other.elems):
                return False
            for el in self.elems:
                if el not in other.elems or not np.all(np.isclose(self[el], other[el])):
                    return False
            return True
...
def find_elem_with_vertex(vnode, position, elems_nodes):
    ...
    try:
        return next(
            e for e, nodes in enumerate(elems_nodes) if nodes[position] == vnode
        )
    except StopIteration:
        raise ValueError(
            "No element with node {} in position {}".format(vnode, position)
        )
