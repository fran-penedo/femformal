...
def draw_pde_trajectories(
    ...
    d_min, d_max = (
        np.amin([np.amin(ds) for ds in dss]),
        np.amax([np.amax(ds) for ds in dss]),
    )
...
def _update_func_disp_2d(ax, ds, mesh, ts, apcs, labels, perts, ds_t, mesh_t, **kwargs):
    ...
    if apcs is not None:
        preds = _update_func_preds_2d(
            apcs, ax, labels, mesh, mesh_t, ds_t, perts, corrections_shown
        )
